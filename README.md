...
PeerVault> list                  # List all stored files
PeerVault> status               # Show connection status
PeerVault> clean                # Clean local storage
PeerVault> quit                 # Exit
```
...
PeerVault organizes files in a dedicated storage directory to keep your project clean:
- `storage/node_<port>_<timestamp>/` - Contains all files for a specific node
- Storage directories are automatically excluded from git tracking
- Each file is stored using content-addressed storage (CAS) with its hash as the filename
- Filename mappings are maintained for easy retrieval
